...
def do_alignments(read_type, args, dbname, ref_fasta_hash,fastqhash,bwaclassrunner,connection_pool,basename,basenameid):
...
# --------------------------------------------------------------------------
def process_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor,fastqhash):


    if read_type == 4:
        return nanonet.process_nanonet_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor, fastqhash)
    if read_type in [1,2,3]:
        return metrichor.process_metrichor_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor,fastqhash)
    if read_type == 6:
        return minknow.process_integratedRNN_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor, fastqhash)
...
def process_model_data(args, basecalldirs, hdf, db, cursor):
...
def getBasenameData(args, read_type, hdf):
    if read_type in [1,2,3]:
        ...
        return nanonet.getNanonetBasenameData(args, read_type, hdf)
    else:
...
def process_basecalledSummary_data(args, read_type, basename, basenameid, basecalldirs, passcheck, tracking_id_hash, general_hash, hdf, db,cursor):

    if read_type in [1,2,3]:
       return metrichor.process_metrichor_basecalledSummary_data(args, read_type, basename, basenameid, basecalldirs, passcheck, tracking_id_hash, general_hash, hdf, db,cursor)
    elif read_type in [4]:
...
def process_fast5(
    ...
    if read_type == 6:
        ...
        process_minKNOW_v1_0_fast5( read_type, passcheck, db, connection_pool, args, ref_fasta_hash, filepath, hdf, dbname, cursor,bwaclassrunner, )

    if read_type == 4:
        if args.verbose == "high":
            print "Nanonet File...."

        process_nanonet_fast5( read_type, passcheck, db, connection_pool, args, ref_fasta_hash, filepath, hdf, dbname, cursor,bwaclassrunner)
...
def process_nanonet_fast5(
    ...
    _, basecalldirs, configdata, read_info_hash  = \
    ...
    if basename != '':
        process_basecalledSummary_data(args, read_type
            , basename, basenameid , basecalldirs , passcheck, tracking_id_hash, general_hash, hdf, db,cursor)

    # PROCESS BARCODE DATA ...
    process_barcode_data(args, hdf, db, cursor,basenameid)

    # PROCESS MODEL DATA  ...
    process_model_data(args, basecalldirs, hdf, db, cursor)
    ...
    process_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor,fastqhash)

    # DO ALIGNMENTS ....
    #do_alignments(read_type, args, dbname, ref_fasta_hash)
    do_alignments(read_type, args, dbname, ref_fasta_hash,fastqhash,bwaclassrunner,connection_pool,basename,basenameid)
...
def process_metrichor_basecalled_fast5(
    ...
    # GET BASENAME AND CONFIGDATA ...
    basename, basecalldirs, configdata = \
                getBasenameData(args, read_type, hdf)
    read_info_hash = {}
...
def  process_minKNOW_v1_0_fast5( read_type, passcheck, db, connection_pool, args, ref_fasta_hash, filepath, hdf, dbname, cursor,bwaclassrunner):
