...
def mkField((s,(t,n))): 
        s = fixSpaces(s)
        if t=='txtdn': field = s+' TEXT DEFAULT NULL'
        if t=='txtnn': field = s+' TEXT NOT NULL'
        if t=='txt': field = s+' TEXT('+str(n)+') NOT NULL'
        if t=='int': field = s+' INT('+str(n)+') NOT NULL'
        if t=='auto': field = s+' INT('+str(n)+') NOT NULL AUTO_INCREMENT, PRIMARY KEY('+s+')'
        if t=='vc': field = s+' VARCHAR('+str(n)+') NOT NULL'
        return field
