...
# ---------------------------------------------------------------------------
def process_integratedRNN_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor, fastqhash):
    ...
    for (readtype, location) in readtypes.iteritems():
        ...
        if location in hdf:
            ...
            if location + 'Alignment' in hdf:  # so its 2D
                ...
                mysql_load_from_hashes(args, db, cursor, readtype, event_hash)
