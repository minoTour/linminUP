...
def process_metrichor_readtypes(args, read_type, basename, basenameid, basecalldirs, tracking_id_hash, general_hash, read_info_hash, passcheck, hdf, db, cursor,fastqhash):
    ...
    for (readtype, location) in readtypes.iteritems():
        if location in hdf:
            ...
            if location + 'Alignment' in hdf:  # so its 2D
                ...
                events_hash = calcTimingWindows(events_hash, start_time, end_time, g_start_time, g_end_time)
...
def getMetrichorBasenameData(args, read_type, hdf):
    ...
    try:
        ...
        print "process_metrichor_basecalled_fast5(): ERROR: Basecalled file without basecall data... " # , basename
        return '',[],None

    result = (basecalldir, basecalldir1, basecalldir2)

    return basename, result,  configdata # , {}


#--------------------------------------------------------------------------------
def process_metrichor_basecalledSummary_data(args, read_type, basename, basenameid, basecalldirs, passcheck, tracking_id_hash, general_hash, hdf, db,cursor):
    if basecalldirs==[]: return ()
    basecalldir, basecalldir1, basecalldir2 = basecalldirs
